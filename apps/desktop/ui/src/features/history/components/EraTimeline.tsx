...
function EventNode({
      ...
      return (
            <li className={clickable ? "is-clickable" : "is-static"}>
                  <span className="history-era-timeline-year">
                        {compactYear(event.year)}
                  </span>
                  <i aria-hidden="true" />
                  <button
                        type="button"
                        ...
                        title={event.note}
                        onClick={() => {
                              if (clickable && event.nodeId)
                                    onOpenNodeId(event.nodeId, event.title);
                              else
                                    onInfo(
                                          `「${event.title}」的独立资料正在整理中，可以从相关人物与事件继续探索。`,
                                    );
                        }}
                  >
                        <b>{event.title}</b>
