...
import { ArrowRight, BookmarkSimple, CaretDown, CaretLeft, CaretUp, Compass, MagnifyingGlass } from "@phosphor-icons/react";
...
import { DEFAULT_ERA_INDEX, ERA_ICONS, clampEraIndex, historyEras, type HistoryEra } from "./historyEras";
...
function durationOf(era: HistoryEra) { return Math.max(1, era.endYear - era.startYear); }

const WHEEL_ITEM_H = 88;
const WHEEL_H = 440;
const SWITCH_LOCK_MS = 380;

function wheelOffset(index: number) { return (WHEEL_H - WHEEL_ITEM_H) / 2 - index * WHEEL_ITEM_H; }

/** 左侧：滚轮式历史时间选择器。 */
function EraWheel({ active, currentIndex, onSelect }: { active: boolean; currentIndex: number; onSelect: (index: number) => void }) {
  ...
  return (
    <fieldset className="history-wheel" ref={wheelRef} aria-label="历史时代滚轮">
      <button type="button" className="history-wheel-step up" aria-label="上一个时代" onClick={() => onSelect(clampEraIndex(currentIndex - 1))}><CaretUp size={18} /></button>
      <div className="history-wheel-viewport">
        <div className="history-wheel-track" style={{ transform: `translateY(${wheelOffset(currentIndex)}px)` }}>
          {historyEras.map((era, index) => {
            ...
            return (
              <div key={era.id} className={`history-wheel-item${dist === 0 ? " is-current" : ""}`} style={{ height: WHEEL_ITEM_H, opacity, transform: `scale(${scale})` }} aria-hidden={dist !== 0}>
      ...
      <button type="button" className="history-wheel-step down" aria-label="下一个时代" onClick={() => onSelect(clampEraIndex(currentIndex + 1))}><CaretDown size={18} /></button>
    </fieldset>
  );
}

/** 右侧：当前时代详情。 */
function EraPanel({ era, onOpen }: { era: HistoryEra; onOpen: (era: HistoryEra) => void }) {
  const hasInk = era.id === "tang";
  return (
    <article className="history-era-panel">
      {hasInk ? <img className="history-era-panel-ink" src={tangPalaceImage} alt="" aria-hidden="true" /> : null}
      <div className="history-era-panel-head">
        <div>
          <span className="history-era-kicker">当前时代</span>
          <h2>{era.name}</h2>
          <p className="history-era-range">{era.yearLabel} · 共 {durationOf(era)} 年</p>
        </div>
        <button type="button" className="history-era-open" onClick={() => onOpen(era)}>展开详情 <ArrowRight size={15} /></button>
      </div>
      <div className="history-era-tags">{era.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
      <p className="history-era-overview">{era.description}</p>
      <div className="history-era-facts">
        {era.facts.map((fact) => {
          const Icon = ERA_ICONS[fact.icon];
          return <div key={fact.label}><Icon size={22} weight="duotone" /><p><small>{fact.label}</small><b>{fact.value}</b><span>{fact.caption}</span></p></div>;
        })}
      </div>
      <section className="history-era-highlights">
        <header><b>精彩速览</b><span>事件 · 人物 · 文化</span></header>
        <div>{era.highlights.map((highlight) => <button type="button" key={highlight.eyebrow} onClick={() => onOpen(era)}><small>{highlight.eyebrow}</small><b>{highlight.title}</b><span>{highlight.description}</span></button>)}</div>
      </section>
    </article>
  );
}
...
/** 底部：中国历史总时间轴。 */
function EraRail({ currentIndex, onSelect }: { currentIndex: number; onSelect: (index: number) => void }) {
  return (
    <nav className="history-era-rail" aria-label="中国历史总时间轴">
      ...
      <div className="history-era-rail-track">
        {historyEras.map((era, index) => (
          <button type="button" key={era.id} className={index === currentIndex ? "selected" : ""} onClick={() => onSelect(index)} aria-current={index === currentIndex ? "true" : undefined}>
            <i aria-hidden="true" />
            <b>{era.name}</b>
...
function NodeChip({ node, onOpen }: { node: HistoryNode; onOpen: (id: string) => void }) {
  return <button type="button" className="history-node-chip" onClick={() => onOpen(node.id)}><small>{KIND_LABELS[node.kind]}</small><b>{node.title}</b><span>{node.summary}</span></button>;
}

function eraForNode(node: HistoryNode): HistoryEra | null {
  return historyEras.find((era) => era.id === node.id || era.id === node.period_id) ?? null;
}

/** 王朝详情：复用首页时代资料，形成真正可阅读的时代导读。 */
function EraStudyGuide({ era }: { era: HistoryEra }) {
  return <section className="history-study-guide">
    <div className="history-study-lead">
      <div><small>时代导读</small><h2>先建立一张 {era.name} 的时代地图</h2></div>
      <p>{era.description}</p>
    </div>
    <div className="history-study-facts">
      {era.facts.map((fact) => {
        const Icon = ERA_ICONS[fact.icon];
        return <article key={`${fact.label}-${fact.value}`}><Icon size={20} weight="duotone" /><div><small>{fact.label}</small><b>{fact.value}</b><span>{fact.caption}</span></div></article>;
      })}
    </div>
    <section className="history-study-highlights">
      <header><h2>从三条线索开始</h2><span>事件 · 人物 · 文化</span></header>
      <div>{era.highlights.map((highlight) => <article key={highlight.eyebrow}>
        <small>{highlight.eyebrow}</small><b>{highlight.title}</b><p>{highlight.description}</p>
      </article>)}</div>
    </section>
    <div className="history-study-tags"><span>本页关键词</span>{era.tags.map((tag) => <b key={tag}>{tag}</b>)}</div>
  </section>;
}

function EraContext({ era }: { era: HistoryEra }) {
  return <aside className="history-era-context">
    <div><small>所属时代</small><b>{era.name}</b><span>{era.yearLabel}</span></div>
    <p>{era.description}</p>
    <div className="history-era-context-tags">{era.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
  </aside>;
}
...
interface HistoryPageProps { active: boolean; setNotice: (message: string) => void; intent?: { id: string; nonce: number } | null; }
...
export function HistoryPage({ active, setNotice, intent }: HistoryPageProps) {
  ...
  const loadHome = useCallback(async () => {
    ...
    setLoading(true);
    try { setHome(await invoke<HistoryHome>("history_home", { cursor: 0 })); } catch (error) { setNotice(errorMessage(error)); } finally { setLoading(false); }
  }, [setNotice]);
  ...
  const openDetail = useCallback(async (id: string) => {
    if (!isTauriRuntime()) return;
    setLoading(true);
    try { const value = await invoke<HistoryDetailView | null>("history_detail", { id }); if (value) { setDetail(value); setGroups([]); setQuery(""); } } catch (error) { setNotice(errorMessage(error)); } finally { setLoading(false); }
  }, [setNotice]);
  ...
  const openEraDetail = useCallback(async (era: HistoryEra) => {
    if (!isTauriRuntime()) return;
    setLoading(true);
    try {
      const value = await invoke<HistoryDetailView | null>("history_detail", { id: era.id });
      if (value) { setDetail(value); setGroups([]); setQuery(""); }
      else { setQuery(era.name); await searchFor(era.name); }
  ...
  const selectEra = useCallback((index: number) => setCurrentEraIndex(clampEraIndex(index)), []);
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowUp") { event.preventDefault(); setCurrentEraIndex((index) => clampEraIndex(index - 1)); }
      else if (event.key === "ArrowDown") { event.preventDefault(); setCurrentEraIndex((index) => clampEraIndex(index + 1)); }
    };
  ...
  if (detail) {
    content = <article className="history-detail">
      <button type="button" className="history-back" onClick={() => setDetail(null)}><CaretLeft size={16} />返回探索</button>
      <header><div><small>{KIND_LABELS[detail.document.node.kind]} · {range(detail.document.node.start_year, detail.document.node.end_year)}</small><h1>{detail.document.node.title}</h1><p>{detail.document.node.summary}</p></div><button type="button" className={`history-save${favorite ? " saved" : ""}`} onClick={() => void toggleFavorite()}><BookmarkSimple size={18} weight={favorite ? "fill" : "regular"} />{favorite ? "已收藏" : "收藏"}</button></header>
      {detailEra && detail.document.node.kind === "dynasty" ? <EraStudyGuide era={detailEra} /> : null}
      {detailEra && detail.document.node.kind !== "dynasty" ? <EraContext era={detailEra} /> : null}
      <DetailSections detail={detail.document.detail} />
      <section className="history-detail-section"><h2>继续探索</h2><div className="history-relations">{detail.relations.map(({ relation, node }) => <button type="button" key={`${relation.from_id}-${relation.to_id}-${relation.kind}`} onClick={() => void openDetail(node.id)}><small>{RELATION_LABELS[relation.kind]}</small><b>{node.title}</b><span>{relation.note}</span><ArrowRight size={15} /></button>)}</div></section>
      <section className="history-detail-section"><h2>资料来源</h2><div className="history-sources">{detail.sources.map((source) => <button type="button" key={source.id} onClick={() => { if (isTauriRuntime()) void openUrl(source.url); }}><b>{source.title}</b><span>{source.authority} · {source.source_type}</span><ArrowRight size={14} /></button>)}</div></section>
    </article>;
  } else if (groups.length) {
    content = <section className="history-results"><header><p>搜索结果</p><button type="button" onClick={() => { setGroups([]); setQuery(""); }}>清除</button></header>{groups.map((group) => <section key={group.kind}><h2>{KIND_LABELS[group.kind]}</h2><div className="history-node-grid">{group.items.map((node) => <NodeChip key={node.id} node={node} onOpen={(id) => void openDetail(id)} />)}</div></section>)}</section>;
  } else if (home) {
    content = <div className="history-explorer">
      <div className="history-explorer-main">
        <EraWheel active={isHome} currentIndex={currentEraIndex} onSelect={selectEra} />
        <EraPanel era={currentEra} onOpen={(era) => void openEraDetail(era)} />
      </div>
      <EraRail currentIndex={currentEraIndex} onSelect={selectEra} />
      <p className="history-chronology-hint">滚动鼠标或使用 ↑ ↓ 切换时代 · 点击底部时间轴可跳转</p>
    </div>;
  } else {
  ...
  return <div className={`history-page${isHome ? " history-home" : ""}`}>
    {isHome ? <div className="history-home-masthead"><header className="history-heading"><p>HISTORY EXPLORER</p><h1>中国历史</h1><span>滚动时光之轮，探索中华文明的演进脉络</span></header>{searchForm}</div> : <><header className="history-heading"><p>HISTORY EXPLORER</p><h1>探索中国历史</h1><span>沿着时间、人物、事件与地点继续前行。</span></header>{searchForm}</>}
    {content}
  </div>;
}
