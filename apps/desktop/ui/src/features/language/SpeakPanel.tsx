...
export function SpeakPanel({ language, setNotice }: SpeakPanelProps) {
    ...
    return (
      <div className="lang-panel">
        <section className="lang-card">
          <h3>
            口语 <small>Speaking</small>
          </h3>
          <p className="lang-muted">
            该语言暂无例句（请先安装 Starter Pack 或导入 Tatoeba）。
          </p>
        </section>
      </div>
    );
  ...
  return (
    <div className="lang-panel lang-speak">
      <section className="lang-card">
        <header className="lang-card-head">
          <h3>
            口语练习 <small>Speaking · Tatoeba #{target.sentence_id}</small>
          </h3>
          <button className="lang-link" onClick={() => void pickTarget()}>
            换一句
          </button>
        </header>
        ...
        <div className="lang-row-actions">
          <button
            ...
            onClick={() => void speak(target.text, target.language)}
            title="播放目标句（TTS）"
          >
            <Play size={16} /> 播放
          </button>
          {recording ? (
            <button className="lang-danger" onClick={stopRecording}>
              <Stop size={16} /> 停止
            </button>
          ) : (
            <button
        ...
        <label className="lang-field">
          <span>
            转写文本（V1：手动输入，或将来接入本地/云端 STT Provider）
          </span>
          <textarea
            value={transcript}
        ...
        {recordedMs > 0 ? (
          <small className="lang-muted">
            录音时长 {Math.round(recordedMs / 1000)}s
          </small>
        ) : null}

        <button className="lang-primary" onClick={() => void evaluate()}>
          评分
        </button>

        {score ? (
          <section className="lang-score">
            <h4>反馈</h4>
            <ul>
              <li>
                准确性 Accuracy <b>{score.accuracy}</b>
              </li>
              <li>
                完整性 Completeness <b>{score.completeness}</b>
              </li>
              <li>
                流利度 Fluency <b>{score.fluency}</b>
              </li>
            </ul>
            <p className="lang-muted">
              基于 Missing / Wrong / Extra Word 与录音时长；不做口音评分（V1
              范围）。
            </p>
          </section>
        ) : null}
