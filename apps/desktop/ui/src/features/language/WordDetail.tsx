...
export function WordDetailView({
  ...
  const readText = () => {
    ...
    if (!speak(text, item.language))
      setNotice("当前设备无可用的 TTS 语音（不影响其他功能）。");
  };

  return (
    <div className="lang-detail-overlay" role="dialog" aria-label="词条详情">
      <div className="lang-detail">
        <header className="lang-detail-head">
          ...
          <span className="lang-state-chip">
            {state ? stateLabel(state.state) : "NEW"}
          </span>
          <button
            onClick={() => void toggleFavorite()}
            title={favorite ? "取消收藏" : "收藏"}
            className={favorite ? "lang-fav on" : "lang-fav"}
          >
        ...
        <section className="lang-word-head">
          ...
          <div className="lang-row-actions">
            <button onClick={readText} title="朗读">
              <Play size={15} />
              发音
            </button>
          </div>
        ...
        {kanji ? (
          <section className="lang-card">
            <h3>
              汉字信息 <small>KANJIDIC2</small>
            </h3>
            <ul className="lang-kanji">
              {kanji.stroke_count == null ? null : (
                <li>笔画 {kanji.stroke_count}</li>
              )}
        ...
        <section className="lang-card">
          <h3>
            释义{" "}
            {meanings.length > 0 ? <small>{meanings.length} 义项</small> : null}
          </h3>
          {meanings.length === 0 ? (
            <p className="lang-empty">
              暂无释义{source ? `（来源 ${source.name} 仅提供发音/转写）` : ""}
              。
            </p>
          ) : (
            <ol className="lang-meanings">
              {meanings.map((meaning) => (
                <li key={meaning.id}>
                  ...
                  {meaning.raw && meaning.raw !== meaning.gloss ? (
                    <small>{meaning.raw}</small>
                  ) : null}
                </li>
              ))}
            </ol>
        ...
        {sentences.length > 0 ? (
          <section className="lang-card">
            <h3>
              相关句子 <small>Tatoeba</small>
            </h3>
            <ul className="lang-examples">
              {sentences.map((sentence) => (
                <li key={sentence.sentence_id}>
                  <button
                    className="lang-sentence"
                  ...
                  <small className="lang-license">
                    {sentence.license} · #{sentence.sentence_id}
        ...
        <section className="lang-card">
          ...
          <div className="lang-state-actions">
            {(
              ["new", "learning", "review", "mastered"] as LearningStateKind[]
            ).map((kind) => (
              <button
                key={kind}
                className={state?.state === kind ? "active" : ""}
                onClick={() => void setState(kind)}
              >
                {stateLabel(kind)}
              </button>
            ))}
        ...
        {source ? (
          <footer className="lang-source">
            <Star size={13} />
            <div>
              <b>{source.name}</b>
              <small>
                {source.dataset_version} · {source.license.kind}
                {source.license.attribution_required ? " · 需署名" : ""}
                {source.license.commercial_use_allowed
                  ? " · 可商用"
                  : " · 非商用"}
              </small>
            </div>
