...
export function LibraryPanel({
  ...
  return (
    <div className="lang-panel lang-library">
      <section className="lang-card">
        ...
        {progress ? (
          <ul className="lang-progress">
            <li>
              已学习 <b>{progress.total}</b>
            </li>
            <li>
              学习中 <b>{progress.learning}</b>
            </li>
            <li>
              已掌握 <b>{progress.mastered}</b>
            </li>
            <li>
              复习次数 <b>{progress.reviews}</b>
            </li>
            <li>
              收藏 <b>{progress.favorites}</b>
            </li>
          </ul>
        ) : (
      ...
      <section className="lang-card">
        <header className="lang-card-head">
          <h3>
            <Heart size={16} />
            收藏 <small>{favorites.length}</small>
          </h3>
        </header>
        {favorites.length === 0 ? (
        ...
        {favorites.length === 0 ? null : favorites.filter(
            (item) => item.language === language,
          ).length === 0 ? (
          <p className="lang-muted">
            （当前语言 {language} 没有收藏；全部收藏 {favorites.length} 条。）
          </p>
        ) : null}
      </section>
      ...
      <section className="lang-card">
        <header className="lang-card-head">
          <h3>
            <Star size={16} />
            数据来源与许可 <small>Settings → Language Data</small>
          </h3>
        </header>
        <p className="lang-muted">
          所有词条均可追溯来源（#5/#90）；Attribution
          展示如下，许可能力位可展开。
        </p>
          ...
          {sources.map((info) => (
            <li key={info.source.id}>
              <div className="lang-source-row">
                ...
                <span>
                  {info.source.license.kind}
                  {info.source.license.attribution_required ? " · 需署名" : ""}
                </span>
                <small>
                  {info.source.dataset_version} · {info.item_count} 条
                </small>
              </div>
              <p className="lang-muted">{info.source.attribution}</p>
              {info.manifest ? (
                <small className="lang-license">
                  清单：{info.manifest.name}（
                  {info.manifest.imported_at &&
                    new Date(
                      info.manifest.imported_at * 1000,
                    ).toLocaleDateString()}
                  ）
                </small>
              ) : null}
      ...
      <section className="lang-card">
        ...
        <p className="lang-muted">
          {(["new", "learning", "review", "mastered"] as const)
            .map(stateLabel)
            .join(" → ")}
          ； 学习状态与词典数据分离存储，#59。
        </p>
