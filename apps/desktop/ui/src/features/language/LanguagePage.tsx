...
export function LanguagePage({ active, setNotice, intent }: LanguagePageProps) {
  ...
  const openDetail = useCallback<OpenDetail>(
    (id: string | null, reset: boolean) => {
      if (reset) {
        setOpenIntent(null);
        setSelected(null);
        return;
  ...
  if (loading) {
    return (
      <div className="page-scroll lang-page">
        <p className="lang-empty">加载语言数据…</p>
      </div>
    );
  }
    ...
    return (
      <div className="page-scroll lang-page">
        <header className="lang-welcome">
          ...
          <p>
            已内置真实数据子集（Open English WordNet · CMUdict · JMdict ·
            KANJIDIC2 · CC-CEDICT · words.hk · CC-Canto ·
            Tatoeba），来源与许可完整可溯。
          </p>
        </header>
        <div className="lang-card lang-install-card">
          ...
          <p>
            导入约 3 千条带 attribution
            的真实词条与例句，完全离线可用；完整数据包可稍后通过
            <code> language-data import … </code>（见
            docs/language/DATA_SOURCES.md）导入。
          </p>
          <button
  ...
  return (
    <div className="page-scroll lang-page">
      ...
      {tab === "listen" && (
        <ListenPanel language={language} setNotice={setNotice} />
      )}
      {tab === "speak" && (
        <SpeakPanel language={language} setNotice={setNotice} />
      )}
      {tab === "library" && (
        <LibraryPanel
          language={language}
