...
export function ReviewPanel({
      ...
      <div className="lang-panel">
        <section className="lang-card">
          ...
          <p className="lang-muted">
            本语言当前没有到期/新词卡片。去 Explore 搜索词条并查看详情，或在
            Today 查看新词配额。 （复习调度为本地 SRS，不联网。）
          </p>
          <button className="lang-primary" onClick={() => void loadNext()}>
            重新检查
          </button>
        </section>
      </div>
  ...
  return (
    <div className="lang-panel lang-review">
      <section className="lang-card">
        <header className="lang-card-head">
          <h3>
            复习卡片{" "}
            <small>
              {language.toUpperCase()} · {stateLabel(card.state)}
            </small>
          </h3>
          <button
            className="lang-link"
            onClick={() => onOpen(card.item.id, false)}
          >
            查看详情
          </button>
        </header>
        ...
        <div className="lang-review-question">
          ...
          {revealed ? (
            <div>
              {card.item.reading ? (
                <p className="lang-roman">{card.item.reading}</p>
              ) : null}
              {card.item.romanization ? (
                <p className="lang-roman">{card.item.romanization}</p>
              ) : null}
              {outcome ? (
                <p className="lang-muted">
                  已评分：
                  {outcome.interval_days >= 1
                    ? `${outcome.interval_days.toFixed(1)} 天后复习`
                    : "今天再复习"}
                  （ease {outcome.ease.toFixed(2)}）
                </p>
              ) : (
            ...
            <button
              ...
              onClick={() => {
                setRevealed(true);
                if (card.item.reading)
                  void speak(card.item.reading, card.item.language);
              }}
            >
              <Eye size={16} />
              显示答案
