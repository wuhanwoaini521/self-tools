...
import type { LanguageCode, LanguageItem, LanguageSearchHit } from "../../types";
...
export function ExplorePanel({ language, onOpen, setNotice }: ExplorePanelProps) {
  ...
  // 语言切换时清空
  useEffect(() => { setHits(EMPTY_RESULTS); setSearched(false); }, [language]);
  ...
  return <div className="lang-panel">
    <div className="lang-search">
      ...
      <input
        ...
        aria-label="搜索语言词条"
      />
      <button onClick={() => void runSearch(query)} disabled={searching || !query.trim()}>搜索</button>
    </div>

    {searched && hits.length === 0
      ? <p className="lang-empty">没有找到「{query}」的结果（安装完整数据包后可获得更大词库）。</p>
      : null}

    {hits.length > 0
      ? <ul className="lang-hit-list">
          {hits.map((hit) => <li key={hit.item.id}>
            <button className="lang-hit" onClick={() => onOpen(hit.item.id, false)}>
              ...
              <small>{matchLabel(hit.matched)} · {hit.item.item_type}</small>
            </button>
          </li>)}
        </ul>
: null}
...
function matchLabel(matched: string): string {
  switch (matched) {
    ...
    case "reading": return "读音";
    case "romanization": return "罗马字";
    case "meaning": return "释义";
    case "text-like": return "包含";
    case "english-index": return "英粤索引";
