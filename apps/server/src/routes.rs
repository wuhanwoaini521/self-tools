...
fn error_response(status: StatusCode, message: String) -> Response {
    (status, Json(ErrorBody { code: "history_error".to_string(), message })).into_response()
}
...
async fn search(State(state): State<AppState>, Query(params): Query<std::collections::HashMap<String, String>>) -> Response {
...
mod tests {
    ...
    use devtoolbox_application::history::{
        HistoryPortError, HistoryQueryPort, HistoryService,
    };
    ...
    impl HistoryQueryPort for FakeHistory {
        ...
        fn get_periods(&self) -> Result<Vec<PeriodResult>, HistoryPortError> {
            if self.fail_all { Err(boom()) } else { Ok(Vec::new()) }
        }
    ...
    fn test_router(fail_all: bool) -> axum::Router {
        router(Arc::new(HistoryService::new(Box::new(FakeHistory { fail_all }))))
    }
    ...
    async fn detail_endpoints_404_with_error_contract() {
        for path in [
            ...
            assert!(body["message"].as_str().unwrap().contains("not found"), "{path}");
        }
    }
