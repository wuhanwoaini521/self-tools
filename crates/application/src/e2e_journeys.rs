...
use devtoolbox_core::personal_ai::{
    AgentRequest, AppContext, ChatModelProvider, ChatRequest, ChatResponse, ChatUsage, ContentPart,
    ModelCapabilities, ProviderError, ToolRisk, ToolSpec,
};
...
use crate::personal_ai::{AgentConfig, InMemorySessionStore, PersonalHub};
...
impl ScriptedProvider {
    ...
    fn push_text(self: &Arc<Self>, text: &str) {
        self.steps.lock().expect("lock").push_back(Ok(ChatResponse {
...
async fn journey_1_hub_home_ask_ai() {
    ...
    assert!(response.session_id.starts_with("once-"));
}
...
async fn journey_10_conversation_survives_across_devices() {
    ...
    assert_eq!(
        reloaded.messages[1].provider.as_deref(),
...
struct InMemoryConversationSpy {
    inner: std::sync::Mutex<std::collections::HashMap<String, Conversation>>,
}
    ...
    for result in &outcome.results {
        assert!(
            result
                .tool_calls
                .iter()
                .all(|call| call.tool != "services.restart"),
...
async fn journey_16_offline_shell_and_degraded_features() {
    ...
    assert_eq!(
        ai.status,
        devtoolbox_core::readiness::ReadinessStatus::NotConfigured
    );
    let backend = report
        .checks
        .iter()
    ...
    assert_eq!(
        backend.status,
        devtoolbox_core::readiness::ReadinessStatus::Ready
    );
}
...
fn journey_18_backup_restore_round_trip_on_fixture() {
    ...
    service
        .register(Arc::new(crate::backup::JsonSource::new(
            "memory", &source, true,
        )))
        .expect("register");
