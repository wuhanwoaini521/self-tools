# History V2 Cutover 前审计（Gate 1）— 历史档案

> ⚠️ **本文是历史快照，不是当前架构描述。** 记录的是 2026-09-10 History V2 产品 Cutover **之前**的状态：
> 当时 legacy V1（`history.db` / `HistoryStore` / `HistoryService` / 5 个 `history_*` 命令 /
> `data/normalized` fallback）仍然存在于代码中。这些内容已在 Cutover 中全部移除，
> 当前架构请以根目录 [`CURRENT_ARCHITECTURE.md`](../../CURRENT_ARCHITECTURE.md) 为准。
>
> 保留原因：本文是驱动 Cutover 的边界审计（Gate 1），含当时的产品完整度快照（地点层缺口、evidence 缺口），
> 对理解 History V2 的数据补全动机仍有参考价值。
>
> 审计日期：2026-09-10
